    ...
    <div class="py-12">
        <div class="mx-auto px-3 md:px-6 lg:px-16">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border py-6 border-gray-200 h-auto lg:w-[90%] w-full mx-auto">
                <form class="w-full lg:px-8 px-4 py-3 my-form" action="{{ route('products.update', $product) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="grid md:grid-cols-2 grid-cols-1 gap-x-6 gap-y-2">
                        <div class="col-span-1 flex flex-col mt-1">
                            ...
                            <x-text-input id="name" class="mt-1 w-full bg-[#fff] text-xs" type="text" name="name" :value="old('name', $product->name)" required autofocus />
                        </div>
                        <div class="col-span-1 flex flex-col mt-1">
                        ...
                        <div class="md:col-span-2 col-span-1 grid md:grid-cols-3 grid-cols-1 gap-x-6 gap-y-2">
                            <div class="col-span-1 flex flex-col mt-1">
                                ...
                                <x-text-input type="text" id="price" class="mt-1 w-full bg-[#fff] text-xs" name="price" :value="old('price', $product->price)" required />
                                <x-input-error :messages="$errors->get('price')" class="mt-2" />
                            </div>

                            <div class="col-span-1 flex flex-col mt-1">
                                <x-input-label for="discount" :value="__('Discount')" />
                                <x-text-input type="text" id="discount" class="mt-1 w-full bg-[#fff] text-xs" name="discount" :value="old('discount', $product->discount)" />
                                <x-input-error :messages="$errors->get('discount')" class="mt-2" />
                            </div>

                            <div class="col-span-1 flex flex-col mt-1">
                                <x-input-label for="quantity" :value="__('Quantity')" />
                                <x-text-input type="text" id="quantity" class="mt-1 w-full bg-[#fff] text-xs" name="quantity" :value="old('quantity', $product->quantity)" />
                        ...
                        <div class="md:col-span-2 col-span-1 grid md:grid-cols-3 grid-cols-1 gap-x-6 gap-y-2">
                            <div class="flex flex-col mt-1">
                                ...
                                <x-text-input type="text" id="weight" class="mt-1 w-full bg-[#fff] text-xs" name="meta[weight]" :value="old('meta[weight]', $product->meta?->weight)" />
                            </div>
                            ...
                            <div class=" flex flex-col mt-1">
                                ...
                                <x-text-input type="text" id="size" class="mt-1 w-full bg-[#fff] text-xs" name="meta[size]" :value="old('meta[size]', $product->meta?->size)" />
                            </div>
                            ...
                            <div class="flex flex-col mt-1">
                                ...
                                <x-text-input type="text" id="color" class="mt-1 w-full bg-[#fff] text-xs" name="meta[color]" :value="old('meta[color]', $product->meta?->color)" />
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col md:col-span-2 mt-1">
                            ...
                            <x-my-textarea name="description" id="markdown-editor" rows="6" class="mt-1 w-full bg-[#fff] text-xs" placeholder="Enter a description for this product to give more details about it.">
                                {!! old('description', $product->description) !!}</x-my-textarea>
                        </div>
